<template>
    <div v-if="!$apollo.$loading" id="i-operator-account-panel">
...
export default class ManagerPanel extends Vue {
    ...
    get title(): string {
        return `[${this.$props.operatorGroupName}] ${this.user.USER_ID} | ${this.user.NAME}`;
    }

    get description(): string {
        return `운영자 [${this.user.NAME}]의 계정을 관리합니다`;
    }

    onClickDeleteManager() {
        this.$confirmDialog.require({
            group: 'deleteConfirmDialog',
            message: `운영자 계정을 삭제하시겠습니까?\n삭제된 계정은 다시는 사용하실 수 없습니다`,
            header: `운영자 계정 [${this.user.USER_ID}] 삭제`,
            position: 'top',
        ...
        this.$apollo
            ...
            .then(({ data: { DeleteUser } }) => {
                if (DeleteUser) {
                    ...
                    eventBus.$emit('refreshOperatorGroupTree');
                    this.$emit('reset');
                }
            })
            .catch((error) => {
    ...
    onClickResetPassword() {
        this.$confirmDialog.require({
            ...
            header: `운영자 계정 [${this.user.USER_ID}] 패스워드 초기화`,
            position: 'top',
            icon: 'pi pi-exclamation-triangle',
            acceptClass: 'p-button-danger',
            blockScroll: false,
...
#i-operator-account-panel::v-deep {
    .i-title {
        font-size: 1.6rem;
