<template>
    <div v-if="productId > 0" id="productPanel">
        ...
        <ScrollPanel class="i-product-scrollpanel">
            <div class="p-grid i-product-scrollpanel">
...
export default class ProductPanel extends Vue {
    ...
    get productInfo(): Array<object> {
        try {
            console.info(this.newProductData.INFO);
            return JSON.parse(this.newProductData.INFO);
        } catch {
            return [];
        }
    }
    ...
    productInterfaceClass(data: any) {
        if (data.INTERFACE.ASSET_CD === this.newProductData.ASSET_CD) {
    ...
    showInterfaceTreePanel(event: Event, intf_data: any) {
        ...
        if (this.$refs.interfaceTreePanel) {
        ...
        if (
            ...
            for (const [idx, intf] of Object.entries(
                ...
                const is = this.productInterfaces.find(
                    (i) => i.PD_INTF_ID === intf.PD_INTF_ID
                );
    ...
    updateProduct() {
        ...
        const insert_product_interfaces = this.productInterfaces
            .filter((intf: any) => intf.PD_INTF_ID !== null)
            .map((intf: any) => {
                return {
                    ID: intf.ID,
        ...
        console.info(insert_product_interfaces);
    ...
    addProductInfo() {
        ...
        const info = JSON.parse(this.newProductData.INFO);
        console.info(info);
        info.push({ key: 'key', value: 'value' });

        this.newProductData.INFO = JSON.stringify(info);
    }
    ...
    deleteProductInterface(idx: number) {
        const prod_intf = this.productInterfaces[idx];
        console.info(prod_intf);
    ...
    get changeProductdInterfacesInfo(): Array<any> {
        const prod_intfs = this.productInterfaces.filter(
            (i) => i.PD_INTF_ID !== null
        );

        const changing_data: Array<any> = [];

        for (const [key, intf] of Object.entries(this.dbProductInterfaces)) {
            const is = this.productInterfaces.find(
                (i) => i.PD_INTF_ID === intf.PD_INTF_ID
            );
            ...
            if (is === undefined) {
                ...
                changing_data.push(data);
            }
        }

        for (const [key, intf] of Object.entries(this.productInterfaces)) {
        ...
        // // by shkoh 20220627: 변경할 Produt Interface 데이터를 정리함
        // if (this.dbProductInterfaces.length > prod_intfs.length) {
        //     // by shkoh 20220627: Case 1. dbProductInterfaces의 숫자가 많은 경우
        // } else if (this.dbProductInterfaces.length === prod_intfs.length) {
        //     // by shkoh 20220627: Case 2. dbProductInterfaces와 productdInterfaces의 숫자가 같은 경우
        // } else if (this.dbProductInterfaces.length < prod_intfs.length) {
        //     // by shkoh 20220627: Case 3. productInterfaces의 숫자가 많은 경우
        // }
