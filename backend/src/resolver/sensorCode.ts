...
import { Ctx, Mutation, Query, Resolver, Args, Publisher, PubSub, Arg, ID } from "type-graphql";
...
export class SensorCodeResolver {
    ...
    async UpdateSensorCode(
        ...
        try {
            ...
            const result = await getRepository(pd_sensor_code).update({ ID: id }, update_data);
